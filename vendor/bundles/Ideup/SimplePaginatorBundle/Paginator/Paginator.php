...
class Paginator
{
    ...
    public function __construct(Request $request)
    {
        ...
        $sortString = $request->query->get('sort');
        $this->loadSortCriteria($sortString);
    ...
    private function loadSortCriteria($sortString, $id = null) {
        if (!$sortString) {
            $this->sortCriterias[md5($id)] = array();
            return false;
        }
        
        $sortTokens = explode(';', $sortString);
        foreach ($sortTokens as $sortToken) {
            try {
                list($column, $direction) = explode(':', $sortToken);
            } catch (\Exception $e) {
                continue;
            }
            $newCriteria = array(
            ...
            $this->sortCriterias[md5($id)][] = $newCriteria;
        }
        return true;
    }
    ...
    public function paginate(Query $query, $id = null)
    {
		...
		$dql = $query->getDQL();
		if (strpos($dql, 'SELECT ') == 0) {
		    $dql = substr($dql, 7);
		}
		$em = $query->getEntityManager();
		$qb = new QueryBuilder($em);
		$qb->addSelect($dql);	
		foreach($this->getSortCriterias($id) as $criteria) {
		    $qb->addOrderBy($criteria['column'], $criteria['direction']);
		}	

		$queryOrdered = $qb->getQuery();
		try {
		    $queryOrdered->getResult();
		} catch (\Exception $e) {
		    $queryOrdered = $query;
		}

		return $queryOrdered->setFirstResult($offset)->setMaxResults($this->getItemsPerPage($id));
    }
     ...
     * @return array
     */
    public function getSortCriterias($id = null)
    {
        return $this->sortCriterias[md5($id)];
    }
}
